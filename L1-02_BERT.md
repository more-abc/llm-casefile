...
1. **P0 事实核查**：独立核查 MLM/NSP 起源、参数量、数据集；新增"Cloze (Taylor 1953) 是 MLM 思想真正源头"、"NSP 是 BERT 原创但被自家后续工作证伪"两条。
2. **P0 自查清单**：去掉所有 `[x]` 打勾，新增 **3 条诚实"未做到"项**（未自己复现预训练、未对 80/10/10 做独立 ablation、未涵盖中文/多语 BERT 谱系）。
3. **P1 篇幅**：从 ~16000 字压到 **~9000 字**；合并"📐 精确版增补"与"🔬 机制深挖"中重复部分（80/10/10、NSP 失效、[CLS] 训练不足），保留唯一权威叙述。
4. **P1 Mermaid fallback**：每张 graph 后追加文字版继承关系列表。
5. **P1 "如果作者偷懒了"**：从实验层面升级为**理论层面**反思——指出最易忽略的 ablation 是"MLM 双向 vs L2R 同算力对照"以及"为什么这一对照决定了'双向'是否合法"。
6. **P2 叙事母题**：放弃 v4 的"生物进化"（评审认为勉强），改用 **"完形填空考试"** 母题，并以"考卷设计 / 考生 / 阅卷标准 / 复盘"贯穿全篇。
7. **P2 代码清理**：删除冗余示例，每段加注释，统一 PyTorch 风格。
8. 顶部新增本修订说明。

保留：四档时长入口、精确事实表、复现 Checklist、物证清单。
...
v4 只指出"BERT vs LTR-only BERT"这条 ablation 的工程价值。本节升级到**理论层面**：
