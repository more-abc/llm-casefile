...
- **P0 事实核查**：
  - 修正 Bradley-Terry 模型起源——并非 Christiano 2017 提出，而是 **Bradley & Terry (1952)** 在《Biometrika》上的经典统计学工作；Christiano 2017 (Deep RL from Human Preferences) 只是把它**引入到深度 RL** 的语境。
  - 收紧"DPO 等价 RLHF"的陈述：论文证明的是**最优解等价**（population-level），不是**有限样本下的训练动力学等价**——这一点在 v4 表述中糊在了一起。
- **P0 自查清单去打勾**：改为诚实的"已做到 / 未做到"两栏，新增 3 条"未做到"项。
- **P1 压缩**：从 ~16000 字缩到 ~9000 字，删去精确版增补与正文重复段落，机制深挖合并到正文。
- **P1 Mermaid fallback**：影响波及图后追加纯文字版继承关系列表。
- **P1 "如果作者偷懒了"**：保留 v4 已被评审认可的"数学推导让 DPO 从工程 trick 上升到理论结果"判断（评审：优秀），并加强为**两层理论反思**——(a) 形式合法性、(b) 论文边界条件诚实度。
- **P2 叙事母题**：保留"侦探破案 / 数学推导=凶器"框架（评审：绝佳），未替换。
- **P2 代码清理**：`dpo_loss` 示例补全 `log_prob` 占位、`batch` 字段说明、stop-gradient 注释；TRL 复现片段去掉冗余参数。
...
**第二层 · 边界条件诚实度（v4 缺失）**：但这道推导也**遮蔽**了一些应被高亮的边界条件——
